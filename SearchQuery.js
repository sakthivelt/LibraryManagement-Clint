...
function UserList() {
    ...
    const getData= async ()=>{
            const data= await axios.get('http://localhost:5000/list').then((item)=>{
                ...
                setUserData2(item.map((item)=>{
    return <Link to={`/UserInfo/${item.RegNo}`} style={{ color: 'gray',textDecoration:'none' }}><ListItem Name={item.Name} Degree={item.Degree} Course={item.Course} RegNo={item.RegNo}/></Link>
    ...
    var data=DDD.filter((item)=>{
        var TorF=true;
        for(let i in SearchQuery){
            if(item[i]===undefined||item[i].toLowerCase()===SearchQuery[i].toLowerCase()||SearchQuery[i]===''){
                TorF=true;
            }else{
                TorF=false;
                break;
    ...
    <div className='search__bar'>
        ...
        <div className="search__bar__item"><TextField id="input-with-icon-grid" label={"Batch"} value={Batch} onChange={(e)=>{setBatch(e.target.value)}}/></div>
    ...
    return <Link to={`/UserInfo/${item.RegNo}`} style={{ color: 'gray',textDecoration:'none' }}><ListItem Name={item.Name} Course={item.Course} RegNo={item.RegNo} Degree={item.Degree}/></Link>
           }):UserData2&&UserData2}
